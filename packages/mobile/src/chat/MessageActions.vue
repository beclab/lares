<template>
  <div class="lares-actions" role="toolbar" :aria-label="t('action.bar')">
    <div class="lares-actions__left">
      <LaresPcTooltip :label="copied ? t('action.copied') : t('action.copy')">
      ...
      <LaresPcTooltip :label="t('action.like')">
        <button
          type="button"
      ...
      <LaresPcTooltip :label="t('action.dislike')">
        <button
          type="button"
      ...
      <LaresPcTooltip :label="t('action.share')">
    ...
    <span v-if="stats" class="lares-actions__stats">{{ stats }}</span>
  </div>
</template>

<script>
import { metricsLine } from "./format.js";
import LaresPcTooltip from "../desktop/ui/PcTooltip.vue";
...
export default {
  ...
  computed: {
    stats() {
      return metricsLine(this.metrics, this.t);
    },
  },
  beforeUnmount() {
...
/* Pointer chrome only: on touch there is no hover to reveal the readings, and
   the mobile reply keeps its bare row of icons. */
.lares-actions__stats {
  display: none;
}
...
@media (hover: hover) {
  ...
  .lares-actions button:hover {
    background: var(--q-background-hover);
    color: var(--q-ink-2);
  }

  .lares-actions button[data-on="true"]:hover {
    color: var(--q-blue-default);
  }

  .lares-actions:hover .lares-actions__stats,
  .lares-actions:focus-within .lares-actions__stats {
    opacity: 1;
  }
}
