...
export const DRIVE_IMPORT_PROMPT = [
  ...
  "Research fetches — git/GitHub APIs, chart YAML/JSON, READMEs, and other non-media references —",
  "are workspace scratch (often under downloads/). They are not Produced; delete them when done.",
  "Do not delete image / video / audio / 3D files the user should still open. Final artifacts the",
  "user keeps (charts, app sources, outputs) belong outside downloads/ via write / edit, or via",
  "workspace_publish when they already exist.",
  "Before the turn closes, Lares re-checks durable Produced paths are still openable. If a check",
  "fails, you will be steered to restore or republish — do not end while naming a broken path.",
  "drive_fetch copies one files-backend file into the workspace when a later edit or transcode",
  "(for example ffmpeg_encode) needs it there. url_fetch and ffmpeg_encode already publish media",
